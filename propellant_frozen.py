...
# FROZEN CHEMISTRY: for a lot of propellants..
...
BASE_OUTDIR  = "isp_results_frozen"
...
# --------- FROZEN Pc/Pe -> eps helper -----------------------
def find_eps_for_pcovpe(cea, Pc, MR, target_pcovpe):
    """
    Solve Pc/Pe = target_pcovpe under FROZEN chemistry from throat to exit.
    Adds frozen=1, frozenAtThroat=1 to get_PcOvPe / get_Pexit.
    """
    base = {"Pc":Pc, "MR":MR, "frozen":1, "frozenAtThroat":1}
...
# --------- frozen version of Isp helper ---------------------
def get_isp_frozen(cea, Pc, MR, eps, pamb_psia):
    """
    Isp with *frozen* chemistry from throat to exit:
      frozen=1, frozenAtThroat=1 in RocketCEA get_Isp.
...
# ------------------------------------------------------------
# --- Core sweep per environment (FROZEN)
# ------------------------------------------------------------
    ...
    for oc, fc, ox_card, fuel_card in pairs:
        ...
        for of in of_vec:
            try:
                ...
                isp = get_isp_frozen(cea, Pc, of, eps, Pe)
            except Exception:
                isp = float("nan")
                eps_fail += 1
            isp_vals.append(isp)
        ...
        print(f"OK: {label:>15s}  peak(fr) ≈ {peak:.2f} s  "
...
def plot_top_segments(results, top_n, path, env_label):
    ...
    ax.set_ylabel("Isp (s)")
    ax.set_title(f"{env_label}: Isp vs O/F @ Pc={PC_PSIA:.0f} psia (frozen)")
...
def run_environment_case(env):
    ...
    xlsx_path     = os.path.join(env_dir, "isp_results_frozen.xlsx")
